...
import { BOOKING_TYPE_LABELS, VEHICLE_CLASS_LABELS } from "@/lib/types";
import { DynamicPricingRule, ServiceRoute, VehicleClassConfig } from "@/lib/admin-operations";
...
export default function PricingPage() {
  ...
  const load = useCallback(async () => {
    try {
      const [ruleData, routeData, vehicleClassData] = await Promise.all([
        apiFetch<DynamicPricingRule[]>("/pricing/admin"),
        apiFetch<ServiceRoute[]>("/admin/routes"),
        apiFetch<VehicleClassConfig[]>("/pricing/admin/vehicle-classes"),
      ]);
      setRules(ruleData);
      setRoutes(routeData);
  ...
  const filteredRules = rules.filter((rule) => {
    ...
    return !normalized || `${route?.nameAr ?? rule.direction ?? ""} ${rule.bookingType} ${rule.vehicleClass} ${rule.currency}`.toLowerCase().includes(normalized);
  });
      ...
      <Shell>
        ...
        <section className="panel">
          ...
          <form className="admin-form-grid" onSubmit={createRule}>
            ...
            <label><span className="label">فئة السيارة</span><select className="input" value={form.vehicleClass} onChange={(e) => setForm({ ...form, vehicleClass: e.target.value as typeof form.vehicleClass })}>{Object.entries(VEHICLE_CLASS_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
