...
import { BookingQuote, BookingType, BOOKING_TYPE_LABELS, Trip } from "@/lib/types";
...
export function BookingForm({ onCreated }: { onCreated?: (booking: Trip) => void }) {
  ...
  function update<K extends keyof BookingFormState>(key: K, value: BookingFormState[K]) {
    ...
    if (["routeId", "bookingType", "passengerCount"].includes(key)) setQuote(null);
    setError("");
  }
  ...
  async function loadQuote() {
    ...
    try {
      const params = new URLSearchParams({ routeId: form.routeId, bookingType: form.bookingType, passengerCount: String(form.passengerCount) });
  ...
  return (
    <form id="booking-card" className="booking-wizard" onSubmit={submit}>
      ...
      <div className="wizard-body">
        ...
        {step === 2 ? <div className="wizard-pane"><div className="pane-heading"><h4>موعد الرحلة</h4><p>{selectedRoute?.requiresFlightDetails ? "بيانات الطائرة مطلوبة لهذا المسار." : "بيانات الطائرة اختيارية لهذا المسار."}</p></div><div className="form-grid wizard-form-grid"><label><span className="label">تاريخ الرحلة</span><input className="input" type="date" min={tomorrow} value={form.travelDate} onChange={(e) => update("travelDate", e.target.value)} required /></label><label><span className="label">عدد المسافرين</span><input className="input" type="number" min={1} max={12} value={form.passengerCount} onChange={(e) => update("passengerCount", Number(e.target.value))} /></label><label><span className="label">وقت الطائرة</span><input className="input" type="time" value={form.flightArrivalTime} onChange={(e) => update("flightArrivalTime", e.target.value)} required={selectedRoute?.requiresFlightDetails} /></label><label><span className="label">رقم الرحلة الجوية</span><input className="input" value={form.flightNumber} onChange={(e) => update("flightNumber", e.target.value)} required={selectedRoute?.requiresFlightDetails} /></label><label><span className="label">عدد الحقائب</span><input className="input" type="number" min={0} max={24} value={form.luggageCount} onChange={(e) => update("luggageCount", Number(e.target.value))} /></label></div></div> : null}
